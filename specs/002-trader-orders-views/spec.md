...
- **Execute rate-only (supersedes institution picker at execute)**: `POST …/orders/{orderId}/execute` accepts only **`executedRate`**. **`institutionCode`** is set at intake by Portfolio Management (`order-institution-constraints`); the server uses the intake institution and sets execution **`counterparty`** from that institution’s **`displayName`**. The trader MUST NOT change counterparty at execute — if they cannot deal with the PM-chosen institution, they reject the order. The server validates the intake institution remains **active** at execute time. Contract: [contracts/api-v1.md](contracts/api-v1.md) v1.7.0+, [contracts/openapi.yaml](contracts/openapi.yaml) `ExecuteOrderRequest`.
...
- Q: Navigation chrome on Assigned / Executed? → A: Primary tab (**ON-CALL** vs **Term**) + sub-tab (Received / Assigned / Executed) MUST stay **visually indicated** as active on every desk queue screen so traders are never ambiguous about product mode or which queue they chose.
- Q: Order details vs primary navigation / back? → A: When the trader opens **order details** from a queue list (**View**), the **same** primary tab (**ON-CALL** vs **Term**) and **same** sub-tab (**Received** / **Assigned** / **Executed**) MUST remain **visually indicated** as active in desk navigation. The control that returns to the queue (**Back to …**) MUST navigate to **that** queue, not a fixed default. Opening order details **without** originating queue context (e.g. pasted URL) is allowed to show **no** queue-specific selection until the user navigates; the back target MAY fall back to the default desk entry (**ON-CALL** > **Received**).
...
The trader application MUST expose **persistent desk navigation** in the **main content area** (below the application header) with two tab tiers: (**1**) primary tabs **ON-CALL** and **Term** (trader-facing labels MUST NOT include the word “Workspace”) and (**2**) sub-tabs **Received**, **Assigned**, and **Executed** scoped to the active primary tab. On every desk queue screen, **both** the active primary tab **and** the active sub-tab MUST be **visually distinguished** (e.g. highlighted or equivalent) so the trader always knows where they are. This MUST hold when navigating directly to Assigned or Executed, not only from Received (no “lost” product-mode context on secondary views). On narrow viewports, both tab tiers MUST remain visible and usable (e.g. horizontal scroll per row); sub-tabs MUST NOT be hidden behind a menu.
...
6. **Given** the trader is viewing **Assigned** or **Executed** under **Term** or **ON-CALL**, **When** they look at desk navigation, **Then** **that** primary tab appears selected **and** **Assigned** or **Executed** appears as the selected sub-tab (not Received).
7. **Given** the trader opened **order details** from a queue (e.g. **Term** > **Assigned** via **View**), **When** they are on the details screen, **Then** desk navigation still shows **that** primary tab and **that** sub-tab as active **and** the back control returns to **that** queue (not another sub-tab or primary tab).
...
- **FR-003a**: Execute MUST accept only **`executedRate`** on the request body. The server MUST use the order’s intake **`institutionCode`** (set at PM reception) and MUST set execution **`counterparty`** from that institution’s **`displayName`**. Execute MUST be rejected when the intake institution is unknown or inactive, or when **`executedRate`** is below the PM **`minimumRate`** when set (contract in [contracts/openapi.yaml](contracts/openapi.yaml), [contracts/api-v1.md](contracts/api-v1.md)).
...
- **FR-013**: While outbound transmit of the execution handoff has **not** succeeded per integration rules, the **Executed** workspace lists MUST expose `handoffStatus` **`PENDING`** or **`FAILED`** on each **`EXECUTED`** row — **`PENDING`** while the transactional outbox has not yet received a Kafka producer ack, **`FAILED`** when publish retries are exhausted — so traders can distinguish **integration/handoff problems** from **`PUBLISHED`** (handoff acked; **awaiting accounting** / books delay). Shapes are contract-first in [contracts/openapi.yaml](contracts/openapi.yaml) (`OrderSummaryResponse.handoffStatus`).
