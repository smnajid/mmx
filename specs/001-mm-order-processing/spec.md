...
2. **Given** the Portfolio Management system submits a valid OnCall Subscription order (including NoticePeriod), **When** the system processes it, **Then** the order appears in the Received OnCall orders list with all submitted details, **and** NoticePeriod is available in the list API (`OrderSummary`) **and** shown as a Notice period column in the OnCall received UI.
...
- **FR-011**: The system MUST display Received Term orders and Received OnCall orders in separate views. The Received **Term** list MUST expose **Tenor** for each row: REST `OrderSummaryResponse` MUST include `tenor` for Term (code from intake) and **null** for OnCall; the Term orders screen MUST show a Tenor column. The Received **OnCall** list MUST expose **NoticePeriod** for each row: REST `OrderSummaryResponse` MUST include `noticePeriod` for OnCall (code from intake, `24H` or `48H`) and **null** for Term; the OnCall orders screen MUST show a Notice period column. The Term received view MUST NOT be required to show Notice period as a dedicated column; the OnCall received view MUST NOT be required to show Tenor as a dedicated column.
