...
Trader-maintained **morning Term rate sheet** for a **trading day**: ingest a CSV, validate against the institution catalog and managed-currency rules, and persist rates at grain **(institutionCode, currency, tenor)** for desk **reference only**. Exposed via contract-first REST under `specs/005-term-rate-settings/contracts/openapi.yaml` and Angular settings UI at `/settings/term-rates`. Rates are **not** applied to order intake, execution, or back-office feeds in this capability.

**Programme decisions (locked):** T-01 row grain; T-02 replace whole day on re-upload; T-03 explicit `tradingDate` column; T-04 trader-only reference; T-05 sample CSV download.
