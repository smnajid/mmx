...
Embeddable Angular library providing a step-by-step order creation wizard that consumes the existing `/api/v1/order-creation/*` endpoints. The widget collects all data needed for a Money Market order and emits a structured payload — the host application is responsible for submission.
...
- **THEN** the widget emits `orderReady` with `{ portfolioNumber: 'PF-001', orderType: 'TERM', currency: 'EUR', operation: 'SUBSCRIPTION', tenor: '3M', institutionCode: 'BNKCO', counterparty: 'BankCo', amount: 1000000, valueDate: '2026-06-10' }`

#### Scenario: Completing an OnCall Increase emits payload with sourceContractNumber

- **WHEN** the user completes the wizard for an OnCall Increase with sourceContractNumber CT-00042
...
#### Scenario: OnCall operations show all four (full flow)

- **WHEN** the wizard reaches the operation step for OnCall/EUR in the normal flow (no contractNumber)
- **THEN** it displays SUBSCRIPTION, INCREASE, DECREASE, and REDEMPTION with their minimum amounts
...
- **THEN** the wizard stores `institutionCode: 'BNKCO'` and `counterparty: 'BankCo'` (display name) and advances

---

### Requirement: Wizard step 6 — Order details (amount, valueDate, minimumRate)

The order details step SHALL present form fields for `amount` (required), `valueDate` (required), and `minimumRate` (optional). Client-side validation SHALL enforce: amount >= minimum amount from step 3, valueDate >= today + 2 calendar days. For OnCall lifecycle operations, the step SHALL also show `sourceContractNumber` (pre-filled from contractNumber input or editable).
...
#### Scenario: OnCall lifecycle shows sourceContractNumber

- **WHEN** the operation is INCREASE, DECREASE, or REDEMPTION
- **THEN** the form displays a sourceContractNumber field (pre-filled from input if available)
...
When the `contractNumber` input is provided, the widget SHALL call `GET /api/v1/order-creation/oncall/contract-info?contractNumber={contractNumber}` on initialization. On success, it SHALL set orderType to ON_CALL, currency and noticePeriod from the response, filter operations to INCREASE/DECREASE/REDEMPTION, and start the wizard at the operation step. On 404, it SHALL show an error with a fallback to the normal flow.
...
- **THEN** the wizard starts at the operation step with orderType=ON_CALL, currency=EUR, noticePeriod=24H, and operations filtered to [INCREASE, DECREASE, REDEMPTION]
...
The mmx trader frontend SHALL include a route at `/dev/widget-playground` (available only in dev mode) that embeds the `OrderCreationWizardComponent` with a configuration panel and event log.
...
The wizard SHALL display a progress indicator showing the current step and total steps. The user SHALL be able to navigate back to any previously completed step. Forward navigation beyond the current step SHALL be disabled until the current step is completed.
