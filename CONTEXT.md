...
**Global account**:
The account reference under which a client **LegalEntity** is known on its hub **LegalEntity's** books — used as the hub-side order's `portfolioNumber` in place of the client's internal portfolio number. Keyed per **`(client LegalEntity, hub LegalEntity, currency)`** (currency-segregated, matching treasury/nostro practice). Resolved at routing time via the **GlobalAccountDirectory** port. If it cannot be resolved for the order's `(client, hub, currency)`, routing fails and the client-side order becomes **Rejected** with a routing reason.
