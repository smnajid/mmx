...
**Routed order**:
An order placed by a **TradingClient**'s Portfolio Management on a delegated Fiduciary institution, which MUST be executed at the connected **TradingHub**. **Every order a TradingClient's PM sends is routed** (a client has only delegated grants and no desk — it cannot place a non-routed order in V1). Routing comes in two flavours depending on whether the TradingClient's connected hub is in the same Organisation as the client:

- **Local routing** (hub is in the same Organisation — e.g. PAR→LOC within LODH): synchronous and atomic in the intake transaction. The use case validates the order against the client's delegated-grant enabled set, resolves the connected hub and the global account via **GlobalAccountDirectory**, creates the **client-side order** and the **hub-side order**, links them by the deterministic routing id, and flips the client-side order `Received → Routed` — all before intake returns. A grant/enabled-set violation or an unresolved global account → intake rejects (client-side `Rejected` with a reason, no hub-side order created).
- **Cross-org routing** (hub is in a different Organisation — e.g. CGD@CGED → LOC@LODH): two asynchronous **legs** over a REST + Kafka transport (see *Leg A* / *Leg B*). The client deployment resolves the hub-side account via **ExternalIdentityGateway**, calls **RemoteRoutingGateway** to send the order to the hub deployment, and transitions the client-side order `Received → Routed` only when the hub accepts (Leg A). Terminal hub outcomes (execute, reject, cancel) propagate back asynchronously via a **RoutingOutcomeV1** Kafka event (Leg B). **Silence is never terminal** — a transport failure or hub unreachability never produces `Rejected`; the gateway's retry/circuit-breaker owns the ops signal and the client-side order stays `Received` until an explicit outcome arrives.

At a **TradingHub**, intake creates the order on the hub's own desk as today (`Received`, no routing).
...
The hub's internal `Assigned` state does NOT propagate to the client-side order. When the hub trader executes the hub-side order, the client-side order is set to **`Executed`**, copying `ExecutedRate`, `ExecutionTime`, `DealingReference` (and contract-number rules per operation); the client-side counterparty is rendered as the client's own delegated-institution display name ("BNP via LOC"), not the hub's native institution. Other terminal outcomes propagate likewise: hub-side **Rejected** → client-side **Rejected**; hub-side **Cancelled** → client-side **Cancelled**. A **routing failure** (e.g. global-account lookup fails, or the hub refuses the routed order) → client-side order becomes **Rejected** with a routing reason.

In **local routing** (same-Organisation), propagation is synchronous in the same transaction as the hub-side transition. In **cross-org routing**, propagation is asynchronous via a **RoutingOutcomeV1** Kafka event published by the hub (Leg B) and applied by the client's **ApplyRemoteOrderOutcomeUseCase**. The event carries only the `routingId` + outcome payload — no internal order UUID crosses the deployment boundary. "Contract validated" is a back-office (Deposits) contract milestone, NOT an MMX order status.
...
Generated **deterministically from the client-side order id**, so a routing retry resolves to the same hub-side order id and the hub dedupes on it (re-routing the same client-side order MUST NOT create a second hub-side order). The hub-side order's own intake idempotency key is this routing id, not the client's `ExternalOrderReference`. In cross-org routing, the hub also dedupes on `(originatingLegalEntityCode, routingId)` to reject a replay from a different client deployment.

**HubLocality** (LOCAL vs REMOTE):
Derived — never stored — by comparing the TradingClient's connected hub's **OrganisationCode** to the deployment's own OrganisationCode. **LOCAL** = same Organisation → synchronous in-process routing. **REMOTE** = different Organisation → cross-org routing via REST + Kafka. Computed at intake time by **HubLocalityResolver** (V1: deployment-role-based — a hub deployment always resolves LOCAL; a client deployment always resolves REMOTE).

**Leg A** (client→hub, outbound routing request):
The client deployment's outbound half of cross-org routing. Steps: (1) **ExternalIdentityGateway** resolves the hub-side portfolio number for `(client, clientPortfolio, hub)`; (2) **RemoteRoutingGateway** sends a `RemoteRoutingRequest` carrying the resolved account + hub-native institution code + routing id to the hub deployment's REST endpoint; (3) the hub's **AcceptRoutedHubOrderUseCase** creates the hub-side order, validates the grant, and commits a Leg-B `ACCEPTED` outbox row in the same transaction; (4) the HTTP response (accept/reject) transitions the client-side order. If the gateway throws (timeout, 5xx, circuit-open), the client-side order stays `Received` — **silence is never terminal**.

**Leg B** (hub→client, asynchronous outcome propagation):
The hub deployment's outbound propagation of terminal hub-side outcomes. When the hub trader executes/rejects/cancels the hub-side order, MMX commits a **RoutingOutcomeV1** Kafka message to the **routing-outcome** topic in the same transaction as the hub-side transition. The client deployment's **RoutingOutcomeKafkaListener** consumes it and applies the outcome via **ApplyRemoteOrderOutcomeUseCase**, transitioning the client-side order. Leg B is the authoritative lifecycle mirror — the hub-side order is the source of truth, and the client-side order converges to it.

**ExternalIdentityGateway**:
Client-deployment port that resolves a TradingClient's hub-side portfolio number (global account) for a `(client LegalEntity, clientPortfolioNumber, hub LegalEntityCode)` triple. Called before Leg A send to ensure the hub receives a hub-native account reference. Backed by an HTTP call to the hub deployment's identity endpoint in V1.

**RemoteRoutingGateway**:
Client-deployment port that sends the routing request to the hub deployment's REST endpoint. Wrapped by a **ResilientRemoteRoutingGateway** that adds retry, exponential backoff, and circuit-breaker. Throws `RemoteRoutingCircuitOpenException` or `RemoteRoutingTransientFailureException` on transport failure — these are caught by the intake use case and the client-side order stays `Received` (silence is never terminal).

**Silence is never terminal** (cross-org routing principle):
A transport failure, timeout, or hub unreachability during Leg A or Leg B never produces a terminal status (`Rejected` / `Cancelled`) on the client-side order. The gateway's retry/circuit-breaker owns the ops signal; the client-side order stays in `Received` (Leg A) or its pre-outcome status (Leg B) until an explicit accept, reject, execute, or cancel outcome arrives from the hub. Only an explicit hub-side outcome (or a routing-failure reject returned synchronously in Leg A) can terminate the client-side order.

**RoutingOutcomeV1** (Kafka event schema):
The wire contract for Leg B. Keyed by `routingId`, it carries `originatingLegalEntityCode`, `outcome` (`ACCEPTED` / `EXECUTED` / `REJECTED` / `CANCELLED`), and outcome-specific payload (rate, dealing reference, reason). Published by the hub to the `mmx.cross-org.outcome-topic` topic; consumed by the client. No internal order UUID crosses the deployment boundary.
...
MMX is deployed **one instance per Organisation** (e.g. one deployment for `LODH`, a separate one for `HSBC`), for fault isolation between organisations. All LegalEntities of an Organisation share that single deployment. A user connects to a specific LegalEntity within the deployment and sees only that LegalEntity's settings. Order routing between LegalEntities of the same Organisation is in-process (local routing). **Cross-Organisation routing** (e.g. CGD@CGED → LOC@LODH) is supported via REST (Leg A) + Kafka (Leg B), where the client deployment calls the hub deployment's REST endpoint and consumes outcome events asynchronously. Each deployment sees only its own order records; the routing id is the sole correlation key across deployments.
...
**TradingClient**:
A **LegalEntity** without its own trading desk. Its orders MUST be routed to its connected TradingHub for execution. A TradingClient is connected to exactly one TradingHub (V1). The connected hub MAY be in a different **Organisation** (cross-org) — in that case, the TradingClient's deployment does not store the hub's LegalEntity; it resolves the connection via cross-org configuration and routes over **Leg A / Leg B**. Its users see Settings only — never the Desk. A LegalEntity is either a TradingHub or a TradingClient, never both (V1).
...
- A **LegalEntity** is either a **TradingHub** or a **TradingClient**, never both (V1). A **TradingClient** is connected to exactly one **TradingHub** (V1), which MAY be in a different **Organisation** (cross-org routing via Leg A / Leg B).
